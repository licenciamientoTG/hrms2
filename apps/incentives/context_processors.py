...
def auto_incentivos_permission(request):
    ...
    if request.user.is_staff:
        tiene_perm = request.user.has_perm('vacations.Modulo_vacaciones')
        return {} if tiene_perm else {'ocultar_vacaciones': True}
    ...
    try:
        ...
        en_travel_inmo = bool(emp.is_active and emp.team and emp.team.strip() == 'Travel Center Inmo')
        es_gerente_ops = (
    ...
    es_jefe_operaciones = es_gerente_ops or es_jefe_zona
    es_limpieza_corporativo = (titulo == 'Auxiliar De Limpieza' and (emp.team or '').strip() == 'Corporativo')
    ocultar_vacaciones = ((en_estacion or en_travel_inmo) and not es_jefe_operaciones) or es_limpieza_corporativo
